...
function SingleTrainingScreen({ navigation, route }) {
  const { year, trainingCategory, trainingTitle, userDetails, title } =
    route.params;
  ...
  const handleStartTraining = async () => {
    ...
    await updateDoc(docRef, {
      trainingsInProgress: arrayUnion(trainingDetails.title),
    });
  };
  ...
  const handleMarkCompleted = async () => {
    ...
    const docRef = doc(db, "users", userId);
    await updateDoc(docRef, {
      completedTrainings: arrayUnion(trainingDetails.title),
    });
    await updateDoc(docRef, {
      trainingsInProgress: arrayRemove(trainingDetails.title),
    });
  };

  // o: passing in a component into another to avoid rewriting loading logic
